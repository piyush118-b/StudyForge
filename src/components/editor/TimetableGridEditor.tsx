  ...
  return (
    <div className="fixed inset-0 z-[400] flex items-center justify-center bg-[#000000]/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-[#13141A]/95 backdrop-blur-3xl border border-red-500/20 rounded-2xl p-6 w-full max-w-sm shadow-[0_24px_64px_rgba(239,68,68,0.2)] ring-1 ring-white/5 flex flex-col pointer-events-auto">
        <h3 className="text-white font-bold mb-1 tracking-wide">Delete Block</h3>
        <p className="text-sm text-slate-400 mb-6">
          Are you sure you want to delete <strong className="text-white font-semibold">"{block.subject}"</strong>? This action cannot be undone.
        </p>

        <div className="flex gap-3 mt-2">
          <button
            onClick={onClose}
            className="flex-1 px-4 py-2 bg-transparent hover:bg-white/5 text-slate-300 rounded-lg text-sm font-semibold transition-colors border border-white/10"
          >
            Cancel
          </button>
          <button
            onClick={onConfirm}
            className="flex-1 px-4 py-2 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white rounded-lg text-sm font-semibold transition-all border border-red-500/20 hover:shadow-[0_0_20px_rgba(239,68,68,0.4)]"
          >
            Delete
          </button>
        </div>
