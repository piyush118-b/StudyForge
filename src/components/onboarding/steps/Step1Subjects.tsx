    ...
    <StepCard title="What are you studying? 📚" description="Add all your subjects so we can plan around them smartly.">
      <div className="space-y-6 pb-6 border-b border-white/5">
        ...
        <div className="space-y-3">
          <Label className="text-[#F0F0F0]/70">Suggested for {userData.branch || "your stream"}</Label>
        ...
        <div className="flex gap-2 relative">
          <Input 
            ...
            className="bg-[#111111] border-white/20 text-[#F0F0F0] rounded-xl h-12 w-full"
            value={customSubject}
          ...
          <Button 
            ...
            className="h-12 px-6 bg-[#1A1A1A] hover:bg-slate-700 text-[#F0F0F0] rounded-xl"
          >
            Add
          </Button>
        </div>
        ...
        {userData.subjects.length > 0 && (
          <div className="space-y-3 pt-4 border-t border-white/10">
            <Label className="text-[#F0F0F0]">Your Subjects ({userData.subjects.length})</Label>
            {userData.subjects.map((sub) => (
              <div key={sub.id} className="bg-[#111111] rounded-xl border border-white/10 overflow-hidden">
                <div 
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <Badge variant="secondary" className="bg-[#1A1A1A] text-[#A0A0A0]">
                      {sub.priority === 'High' ? '🔥 High' : sub.priority === 'Medium' ? '⚡ Med' : '🌱 Low'}
                    </Badge>
                  </div>
                  <div className="flex items-center gap-2">
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-[#606060] hover:text-red-400" onClick={(e) => { e.stopPropagation(); removeSubject(sub.id); }}>
                      <X className="w-4 h-4" />
                    </Button>
                    {expandedId === sub.id ? <ChevronUp className="w-4 h-4 text-[#A0A0A0]" /> : <ChevronDown className="w-4 h-4 text-[#A0A0A0]" />}
                  </div>
                </div>

                {expandedId === sub.id && (
                  <div className="p-4 pt-0 border-t border-white/5 space-y-5 bg-[#111111]/50">
                    
                    {/* Priority */}
                    <div className="space-y-2 mt-4">
                      <Label className="text-[#F0F0F0]/60 text-xs uppercase tracking-wider">Priority / Difficulty</Label>
                      <div className="flex gap-2">
                        {['High', 'Medium', 'Low'].map((p) => (
                          <div
                            key={p}
                            onClick={() => updateSub(sub.id, { priority: p as Subject["priority"] })}
                            className={`cursor-pointer px-4 py-2 rounded-lg border text-sm font-medium transition-all duration-150-all ${sub.priority === p ? 'bg-[#10B981]/20 border-[#10B981] text-indigo-300' : 'bg-slate-950 border-white/10 text-[#A0A0A0] hover:border-white/30'}`}
                          >
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <Label className="text-[#F0F0F0]/60 text-xs uppercase tracking-wider">Lecture Hrs/Wk</Label>
                        <Input 
                          type="number" min={0}
                          className="bg-slate-950 border-white/10 text-[#F0F0F0]"
                          value={sub.lectureHours}
                          onChange={e => updateSub(sub.id, { lectureHours: parseInt(e.target.value) || 0 })}
                        />
                      </div>
                      <div className="space-y-2">
                        <Label className="text-[#F0F0F0]/60 text-xs uppercase tracking-wider">Self-Study Hrs/Wk</Label>
                        <Input 
                          type="number" min={0}
                          className="bg-slate-950 border-white/10 text-[#F0F0F0]"
                    ...
                    {/* Type */}
                    <div className="space-y-2">
                      <Label className="text-[#F0F0F0]/60 text-xs uppercase tracking-wider">Subject Type</Label>
                      <div className="flex flex-wrap gap-2">
                        {SUBJECT_TYPES.map(t => (
                          <Badge
                            key={t}
                            ...
                            className={`cursor-pointer px-3 py-1 border transition-all duration-150-all ${sub.type.includes(t) ? 'bg-teal-500/20 border-teal-500 text-teal-300' : 'bg-slate-950 border-white/10 text-[#A0A0A0] hover:border-white/30'}`}
                          >
                            {t}
                          </Badge>
                        ))}
