  ...
  return (
    <>
      <Draggable handle=".drag-handle" nodeRef={nodeRef} bounds="parent"
        ...
        <div ref={nodeRef} className={`fixed z-[100] w-72 bg-[#1A1A1A]/95 backdrop-blur-xl border ${pc.border} rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden transition-all duration-150-colors duration-500`}>
          ...
          <div className={`drag-handle flex items-center justify-between px-4 py-3 cursor-move bg-[#0A0A0A]/40 border-b ${pc.border}`}>
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => setShowSettings(true)} className="p-1.5 rounded-lg text-[#606060] hover:text-[#F0F0F0] hover:bg-[#222222] transition-all duration-150-all duration-150">
                <Settings className="w-3.5 h-3.5" />
              </button>
              <button onClick={() => store.toggleVisibility()} className="p-1.5 rounded-lg text-[#606060] hover:text-[#F0F0F0] hover:bg-[#222222] transition-all duration-150-all duration-150">
                <X className="w-4 h-4" />
              </button>
            </div>
          </div>
            ...
            <div className="relative w-40 h-40">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 120 120">
                ...
                <circle cx="60" cy="60" r="54" fill="none"
                  ...
                  className="transition-all duration-150-all duration-1000 ease-linear"
                />
              </svg>

              {/* Center */}
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <span className="text-4xl sm:text-5xl font-black text-[#F0F0F0] tracking-tight font-mono tabular-nums w-[140px] text-center inline-block">
                ...
                {store.phase !== 'long_break' && (
                  <div className="flex items-center gap-1.5 mt-2">
                    {Array.from({ length: store.config.sessionsBeforeLongBreak }).map((_, i) => (
                      <div key={i} className={`w-1.5 h-1.5 rounded-full transition-all duration-150-all duration-300 ${i < store.sessionCount ? 'bg-[#10B981] shadow-[0_0_5px_rgba(16,185,129,0.6)]' : 'bg-[#2A2A2A]'}`} />
                    ))}
                  </div>
                )}
              </div>
          ...
          <div className="flex items-center justify-center gap-3 px-6 pb-6">
            <button onClick={store.reset}
              className="w-11 h-11 rounded-xl flex items-center justify-center border border-[#2A2A2A] bg-[#1A1A1A] text-[#A0A0A0] hover:text-[#F0F0F0] hover:bg-[#222222] hover:border-[#333333] transition-all duration-150-all duration-150 active:scale-[0.95] hover:-translate-y-0.5 hover:border-[#333333] hover:shadow-[0_8px_32px_rgba(0,0,0,0.4)] transition-all duration-200">
              <RotateCcw className="w-4 h-4" />
            </button>

            <button onClick={store.isRunning ? store.pause : store.start}
              className={`w-28 h-11 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all duration-150-all duration-150 active:scale-[0.97] ${
                store.isRunning
          ...
          <div className="px-4 py-3 border-t border-[#2A2A2A] bg-[#0A0A0A]/40 mt-auto space-y-2">
            {currentTask ? (
              <div className="flex justify-between items-center group relative">
                ...
                <button onClick={store.unlinkAll} className="opacity-0 group-hover:opacity-100 p-1 text-[#606060] hover:text-[#EF4444] transition-all duration-150-all active:scale-[0.97]">
              ...
              <div className="relative">
                <button onClick={() => setShowTaskLink(!showTaskLink)}
                  className="w-full flex items-center justify-center gap-1.5 py-1.5 text-xs text-[#606060] hover:text-[#10B981] hover:bg-[rgba(16,185,129,0.08)] rounded-lg transition-all duration-150-all duration-150 border border-transparent hover:border-[#10B981]/20">
                  ...
                  <div className="absolute bottom-full left-0 right-0 mb-1 bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] max-h-48 overflow-y-auto z-50 p-1">
                    {tasks.filter(t => t.status !== 'completed' && t.status !== 'cancelled').map(t => (
                      <button key={t.id} onClick={() => { store.linkToTask(t.id, t.subject || undefined); setShowTaskLink(false); }}
                        className="w-full text-left px-3 py-2 text-xs text-[#A0A0A0] hover:bg-[rgba(16,185,129,0.1)] hover:text-[#F0F0F0] rounded-lg transition-all duration-150-all duration-100 truncate">
                        {t.title}
                      </button>
                    ))}
            ...
            <button onClick={() => store.setFocusMode(true)}
              className="w-full py-2 flex items-center justify-center gap-2 text-xs font-semibold text-[#A0A0A0] bg-[#222222] hover:bg-[#2A2A2A] hover:text-[#F0F0F0] rounded-xl transition-all duration-150-all duration-150 border border-[#2A2A2A]">
