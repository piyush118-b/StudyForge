        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-3xl">
          {options.map((opt, idx) => (
            <Link
              ...
              href={opt.href}
              className={`
                group relative bg-[#1A1A1A] border border-[#2A2A2A] rounded-[24px] p-8 flex flex-col
                transition-all duration-300 active:scale-[0.98] ${opt.shadow}
                animate-[forge-fade-in_0.4s_ease-out_forwards]
              `}
              style={{ animationDelay: `${idx * 100}ms` }}
            >
              {/* Subtle Inner Gradient */}
              <div className={`absolute inset-0 rounded-[24px] bg-gradient-to-br ${opt.color} pointer-events-none opacity-50`} />

              <div className="relative z-10 flex flex-col h-full">
                <div className="flex justify-between items-start mb-6">
                  <div className={`w-12 h-12 rounded-2xl ${opt.iconBg} flex items-center justify-center shadow-lg transition-transform duration-300 group-hover:scale-110 group-hover:-rotate-3`}>
                    <opt.icon className={`w-6 h-6 ${opt.iconColor}`} />
                  </div>
                  {opt.premium && (
                    <span className="bg-[rgba(16,185,129,0.1)] text-[#10B981] text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full border border-[#10B981]/20">
                      AI Enabled
                    </span>
                  )}
                </div>

                <h3 className="text-xl font-bold text-[#F0F0F0] mb-2">{opt.title}</h3>
                ...
                <ul className="space-y-2.5 mb-8">
                  {opt.features.map(f => (
                    <li key={f} className="flex items-center text-[13px] text-[#A0A0A0] font-medium">
                      <div className={`w-1.5 h-1.5 rounded-full mr-2.5 ${opt.premium ? 'bg-[#10B981]' : 'bg-[#606060]'}`} />
                      {f}
                    </li>
                  ))}
                </ul>

                <div className={`w-full h-11 rounded-xl flex items-center justify-center font-bold text-sm transition-all duration-200 mt-auto ${opt.btnColor}`}>
